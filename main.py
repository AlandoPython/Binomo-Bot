...
sleep(2)
print('Agora digite a sua meta')
take = int(input('(só números inteiros) R$ '))
sleep(1)
print('Agora digite o valor máximo de perca')
loss = int(input('(só números inteiros) R$ '))
sleep(1)
print('Conectando ao Gráfico...')
sleep(3)
os.system('cls')
mt5.initialize()
print('ROBÔ INICIADO...')
...
saldo_inicial = nav.find_element(By.XPATH,'//*[@id="qa_trading_balance"]').text.strip().replace('R$', '').replace(' ', '').replace('.','').replace(',', '.')
saldo_inicial = float(saldo_inicial)"""
while True:
    #saldo = nav.find_element(By.XPATH,'//*[@id="qa_trading_balance"]').text.strip().replace('R$', '').replace(' ', '').replace('.','').replace(',', '.')
    #saldo = float(saldo)
    #print(saldo)
    ...
    #ema200 = tb.EMA(close,100)
    # ============================================================================
    if high.loc[v-1] >= upper.loc[v-1] and close.loc[v-1] < upper.loc[v-1]:
        put.click()
        cont += 1
    ...
    # APOS CALL/PUT =========================================================================================================================================================================================
        ...
        sleep(espera)
        preco_entrada = nav.find_element(By.XPATH,'//*[@id="qa_trading_tradeHistoryStandardTab"]/option-item[1]/div/div[2]/div[2]/p[2]').text.strip().replace('R$', '').replace(' ', '').replace('.','').replace(',', '.')
        if '₮' in preco_entrada:
            preco_entrada = preco_entrada.replace('₮','')
        preco_entrada = float(preco_entrada)
        preco_resultado = nav.find_element(By.XPATH,'//*[@id="qa_trading_tradeHistoryStandardTab"]/option-item[1]/div/div[2]/div[2]/p[1]').text.strip().replace('R$', '').replace(' ', '').replace('.','').replace(',', '.')
    ...
    elif low.loc[v-1] <= lower.loc[v-1] and close.loc[v-1] > lower.loc[v-1]:
        call.click()
        cont += 1
